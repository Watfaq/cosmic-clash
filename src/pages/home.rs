...
pub fn view_home(app: &AppModel, space_s: u16) -> Element<'_, Message> {
	...
	// Control button
	let control_button = widget::button::text(if app.vpn_is_active { "STOP VPN" } else { "START VPN" })
	...
	let vpn_card = widget::container(
		widget::column::with_capacity(3)
			.push(status_header)
			.push(status_details)
			.push(widget::row::with_capacity(1).push(control_button).width(Length::Fill))
			.spacing(space_s * 2),
	)
	.padding(space_s * 2)
	.class(cosmic::style::Container::Card);
	...
	main_column.height(Length::Fill).into()
}
